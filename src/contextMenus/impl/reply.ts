...
export default class Reply extends ContextMenu<ApplicationCommandType.Message> {
    constructor () {
        super({
            name: 'Reply',
    ...
    async execute(bot: Bot, interaction: MessageContextMenuCommandInteraction<CacheType>): Promise<void> {
        ...
        await bot.tickets.sendMessage(interaction.targetMessage);
        
        interaction.editReply('Sent the message to the user');
    }
}